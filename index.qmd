...
$$
where the amplitudes $\delta^{\mathcal{R}}(\vec k)$ are random variables defined by the dimensionless power spectrum. In general, we are interested in an obervable field $\delta^{\mathcal{x}}$, which related to $\delta^{\mathcal{R}}$ via a transfer function $\Delta^X$.

:::
::: {.column width="50%"}

It is often more convenient to work in spherical cooridnates. By expanding the plane waves in the spherical harmonics, we can rewrite the eingenmodes as 
$$
...
As seen in the 3-torus case, compacts topologies create multiple copies of objects. If any our clonea are situated a distance smaller than diameter of the last scatter surface (LSS), the our observation sphere intersects the clones' spheres, leading to the _self intersections_ of the LSS.
